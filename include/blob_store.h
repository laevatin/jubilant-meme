...
// segment is 1-indexed; segment == 0 is the invalid sentinel.
struct Index {
    uint32_t segment = 0;  // 0 => invalid
...
public:
    struct Options {
        std::string dir;                       // directory holding *.seg files (created if absent)
        uint64_t segment_size      = 256ull << 20;  // seal & rotate past this (must be < 4 GiB)
